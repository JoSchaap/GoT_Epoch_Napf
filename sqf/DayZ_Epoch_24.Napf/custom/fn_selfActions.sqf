...
// ---------------------------------------spawn supplybox Start------------------------------------
if((speed player <= 1) && _canDo) then {
if (s_player_selfspawn < 0) then {
s_player_selfspawn = player addaction[("<t color=""#00ff00"">" + ("Spawn Supplies") +"</t>"),"custom\snap_build\crate.sqf","",5,false,true,"", ""];
};
} else {
player removeAction s_player_selfspawn;
s_player_selfspawn = -1;
};
// ---------------------------------------spawn supplybox End------------------------------------
// ---------------------------------------getgold Start------------------------------------
if((speed player <= 1) && _canDo) then {
if (s_player_selfgold < 0) then {
s_player_selfgold = player addaction[("<t color=""#00ff00"">" + ("Give Goldbar") +"</t>"),"custom\snap_build\gold.sqf","",5,false,true,"", ""];
};
} else {
player removeAction s_player_selfgold;
s_player_selfgold = -1;
};
// ---------------------------------------getgold End------------------------------------
...
_isDeployed = cursorTarget getVariable ["Deployed",false];
	if ((_isDeployed) && ((player distance cursorTarget) < 5)) then {
		_dObj = cursorTarget;
		if (s_player_packOBJ < 0) then {
			s_player_packOBJ = player addaction [("<t color=""#00FF04"">" + ("Pack "+typeOf _dObj+"") +"</t>"),"custom\Deploy\pack.sqf",_dObj,6,false,true,"", ""]
		};
	} else {
